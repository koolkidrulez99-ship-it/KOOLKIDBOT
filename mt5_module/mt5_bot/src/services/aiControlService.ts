...
export const aiControlService = {
  ...
  autoConfigure: (payload: { enabled: boolean; account_login?: number; symbol?: string; volume?: number; scan_seconds?: number }): Promise<AiAutoStatus> =>
  ...
  autoSelectConfigure: (payload: { enabled: boolean; account_login?: number; symbol?: string; enabled_bot_ids?: number[]; mode?: AiAutoSelectMode; scan_seconds?: number }): Promise<AiAutoSelectStatus> =>
    apiRequest<AiAutoSelectStatus>('/api/mt5/ai/auto-select', 'PUT', payload),
  autoSelectExecute: (): Promise<Record<string, unknown>> =>
    apiRequest<Record<string, unknown>>('/api/mt5/ai/auto-select/execute', 'POST', {}),
};
