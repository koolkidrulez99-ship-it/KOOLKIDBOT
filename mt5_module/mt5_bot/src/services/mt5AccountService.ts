...
export const mt5AccountService = {
  ...
  testConnection: (payload: Record<string, unknown>): Promise<ConnectionTestResult> => isSimulation ? simTestAccount(payload) : apiRequest<ConnectionTestResult>('/api/mt5/accounts/test', 'POST', payload, 80000),
  connect: (payload: Record<string, unknown>): Promise<Mt5Account> => isSimulation ? simAddAccount(payload) : apiRequest<Mt5Account>('/api/mt5/accounts/connect', 'POST', payload, 80000),
  ...
  resetBudget: (id: number): Promise<Mt5Account> =>
    isSimulation ? Promise.reject(new Error('Account budgets require bridge mode.')) : apiRequest<Mt5Account>(`/api/mt5/accounts/${id}/budget`, 'PUT', { reset: true }, 15000),
};
