...
function Cover({ open }: { open: (view: EntryView) => void }) {
  return <main className="mt5-public">
    <header className="mt5-public-head"><Brand /><nav><a href="#tools">Capabilities</a><a href="#desk">Workspace</a><button onClick={() => open('login')}>Log in</button><button className="mt5-nav-cta" onClick={() => open('signup')}>Create account</button></nav></header>
