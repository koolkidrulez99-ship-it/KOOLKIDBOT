...
def register_cloud_routes(
    ...
    *,
    ...
    def _token_required_response():
        return jsonify({
            "status": "error",
            "message": "Connect and verify the Deriv API token before starting or saving Cloud Trading.",
    ...
    def cloud_under9_start():
        ...
        status["token_verified"] = bool(ident.get("token_verified"))
        if socketio:
    ...
    def cloud_under9_status():
        ...
        if not username:
            return jsonify({
                ...
                "cloud_enabled": False,
                "running": False,
                "cloud_status": "Connect the exact Deriv API token to verify this Cloud session.",
                "requires_token_verification": True,
        ...
        status["token_verified"] = bool(ident.get("token_verified"))
        status["requires_token_verification"] = not bool(ident.get("token_verified"))
        return jsonify(status)
