(() => {
  let csrf = '', busy = false, initialized = false;
  ...
  async function refresh() {
    try {
      ...
      status.textContent = `Engine: ${h.state} | Connection: ${h.connected ? 'ONLINE' : 'OFFLINE'} | Authorization: ${h.authorized ? 'CONFIRMED' : 'UNCONFIRMED'} | PAT: ${h.pat_configured ? 'Saved' : 'Not configured'} | App ID: ${h.app_id_configured ? 'Saved' : 'Not configured'} | Markets: ${h.markets_monitored || 0} | Ticks: ${h.ticks_received || 0}${h.error ? ' | ' + h.error : ''}${h.test_result ? ' | Test: ' + h.test_result : ''}`;
      if (!initialized && d.policy) {
        el('globalConfirmationEnabled').checked = d.policy.enabled;
        el('globalConfirmationWindow').value = d.policy.window;
        initialized = true;
      }
